...
public class GhostControllerInnovationScene : MonoBehaviour
{
    private UIManager uIManager;
    private PacCatControllerInnovationScene pacCatController;
    ...
    private float defaultPenaltyFactory = 0.3f;

    public Collider2D ghost1Collider;
    public Collider2D ghost2Collider;
    public Collider2D ghost3Collider;
    public Collider2D ghost4Collider;
    ...
    private void LoadObjectsOnStart()
    {
        ...
        if (pacCatObj != null)
        {
            pacCatController = pacCatObj.GetComponent<PacCatControllerInnovationScene>();
        }
        else
        {
    ...
    private void LoadGhosts()
    {
        ...
        ghost1Collider = ghost1.GetComponent<Collider2D>();
        ...
        ghost2Collider = ghost2.GetComponent<Collider2D>();
        ...
        ghost3Collider = ghost3.GetComponent<Collider2D>();
        ...
        ghost4Collider = ghost4.GetComponent<Collider2D>();
    }
    ...
    private IEnumerator LerpGhost1(Vector3 target)
    {
        ...
        if (mapArray[ghost1CurrentRow, ghost1CurrentCol] == 5)
        {
            penaltyFactor = defaultPenaltyFactory;
        }
        if (!isGhost1Dead)
        {
            distanceCovered = 0;
            ...
            while (Vector3.Distance(currentTransform.position, target) > 0.01f)
            {
                if (isGhost1Dead) 
                {
                    Debug.Log(mapArray[-Mathf.RoundToInt(startingPosition.y / stepLength), Mathf.RoundToInt(startingPosition.x / stepLength)] + "next" + mapArray[-Mathf.RoundToInt(target.y / stepLength), Mathf.RoundToInt(target.x / stepLength)]);
                    break;
                }
                distanceCovered += Time.deltaTime * deltaTimeSpeed * penaltyFactor;
        ...
        if (mapArray[ghost1CurrentRow, ghost1CurrentCol] == 5) 
        {
            pacCatController.score = pacCatController.score - 10;
            uIManager.UpdateScore(pacCatController.score);
        }
        mapArray[ghost1CurrentRow, ghost1CurrentCol] = 0;
    ...
    private IEnumerator LerpGhost2(Vector3 target)
    {
        ...
        float lerpFraction;
        float penaltyFactor = 1;
        if (mapArray[ghost2CurrentRow, ghost2CurrentCol] == 5)
        {
            penaltyFactor = defaultPenaltyFactory;
        }
        if (!isGhost2Dead)
        {
            ...
            while (Vector3.Distance(currentTransform.position, target) > 0.02f)
            {
                ...
                {
                    Debug.Log(mapArray[-Mathf.RoundToInt(startingPosition.y / stepLength), Mathf.RoundToInt(startingPosition.x / stepLength)] + "next" + mapArray[-Mathf.RoundToInt(target.y / stepLength), Mathf.RoundToInt(target.x / stepLength)]);
                    mapArray[-Mathf.RoundToInt(startingPosition.y / stepLength), Mathf.RoundToInt(startingPosition.x / stepLength)] = 0;
                    mapArray[-Mathf.RoundToInt(target.y / stepLength), Mathf.RoundToInt(target.x / stepLength)] = 0;
                    break;
                }
                distanceCovered += Time.deltaTime * deltaTimeSpeed * penaltyFactor;
                lerpFraction = distanceCovered / lerpLength;
        ...
        mapArray[ghost2CurrentRow, ghost2CurrentCol] = 0;
    ...
    private IEnumerator LerpGhost3(Vector3 target)
    {
        ...
        float lerpFraction;
        float penaltyFactor = 1;
        if (mapArray[ghost3CurrentRow, ghost3CurrentCol] == 5)
        {
            penaltyFactor = defaultPenaltyFactory;
        }
        if (!isGhost3Dead)
        ...
        mapArray[ghost3CurrentRow, ghost3CurrentCol] = 0;
    ...
    private IEnumerator LerpGhost4(Vector3 target)
    {
        ...
        float lerpFraction;
        float penaltyFactor = 1;
        if (mapArray[ghost4CurrentRow, ghost4CurrentCol] == 5)
        {
            penaltyFactor = defaultPenaltyFactory;
        }
        if (!isGhost4Dead)
        ...
        mapArray[ghost4CurrentRow, ghost4CurrentCol] = 0;
